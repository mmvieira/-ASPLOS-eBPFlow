...
In building eBPFlow, we identified and solved seven challenges issues.
...
To handle state operations in the data plane with an expressive but yet simple model, eBPFlow utilizes key-value data storage that is decoupled from the data memory and can be accessed through function calls in a well-defined API.
...
Our results show that eBFlow can operate at line rate and consumes 75\% less energy than an \emph{idle} CPU.
%We plan to make the eBPFlow implementation publicly available.
