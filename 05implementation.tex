...
\label{secsec:zerocopy}

%\red{The eBPF processor uses the queue (Figure~\ref{fig:06_fig01}) as data memory and does not need to copy the packet to another memory. The processor does not have to wait for all words of the packet to be queued before processing begins. The queue has capacity of 2 MB (64 depth x 256 width) and can be stored up 32 packets of 64 bytes. In the queue there is a field called valid bit that receives $0$ if the word has not arrived and $ 1$ if the word is in the queue.

%When load/store instructions are executed at the EX/MEM processor stage, it is checked if the requested line address has valid bit active (value 1). If the valid bit value is 0, the processor pipeline is blocked until the word is stored in the queue. When the word is stored the valid bit field is updated to 1 and the read or write operation of the requested address is performed, freeing the execution of the pipeline and continuing the execution of the following instructions.

%After all instructions were executed the packet is forwarded, and the valid bit field is updated to 0 by disabling the queue line of the packet.}
...
The eBPF engine can start processing the packet even if the packet did not fully arrive yet.
...
%DM\_FIFO's FSM is thrown when load and store instructions are executed at the processor's pipeline EX/MEM stage.
